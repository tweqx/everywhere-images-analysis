...
int main(int argc, char** argv) {
  ...
  /* Convert the pixels to DCT coefficients. Using a quality of 100 will result in no quantization happening */
...
//              printf("q(q(%d, Q%d * Q%d), %d) \\in {%d, %d}\n", recreation_block_coeffs[i], i, i, q75_luminance_quantization_table[i], reference_block_coeffs[i], reference_block_coeffs[i] ^ 1);
                printf("%d %d %d %d %d\n", recreation_block_coeffs[i], i, q75_luminance_quantization_table[i], reference_block_coeffs[i], reference_block_coeffs[i] ^ 1);
          }
        }
      }
    }
